...
total_lines = count_lines(test_path)
...
with open(test_path, 'r', encoding='utf-8') as f, tqdm(f, total=total_lines, desc='生成候选集') as session_pbar, tqdm(
    ...
    for i, line in enumerate(session_pbar):
        ...
        if (i + 1) % batch_size == 0:
            ...
            df.to_parquet(f"dataset\\test_candidates\\candidates_part_{i // batch_size}.parquet", index=False)
            candidate_rows = []  # 清空缓存
            part_id += 1
            batch_pbar.update(1)
...
if candidate_rows:
    df = pd.DataFrame(candidate_rows)
    df.to_parquet(f"dataset\\test_candidates\\candidates_part_last.parquet", index=False)
    part_id += 1
    batch_pbar.update(1)

print("候选集已分批保存到 dataset/")

files = sorted(glob.glob("dataset\\test_candidates\\candidates_part_*.parquet"))
...
with tqdm(total=len(files), desc="合并候选集文件") as merge_pbar:
    for f in files:
        ...
        if writer is None:
            writer = pq.ParquetWriter("dataset\\test_candidates.parquet", table.schema)
...
print("候选集已成功合并保存到 dataset\\test_candidates.parquet")
